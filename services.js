/* ===================================
   ...
   services.js
   Hero aleatorio + Overview + Cards + Modal Contacto
   =================================== */
...
async function loadHeroBackgrounds() {
    try {
        ...
        console.error('[SERVICES] Error al cargar backgrounds:', error);
        return null;
    }
}
...
async function initHeroBackground() {
    ...
    img.onload = () => {
        SERVICES_DOM.heroBg.style.backgroundImage = `url('${selected.url}')`;
...
function renderCardTools(card) {
    ...
    if (Array.isArray(card.software)) {
        card.software.forEach(swId => {
            // Buscamos el software en el JSON para obtener el nombre
            const sw = getSoftwareById(swId);
            const swName = sw ? sw.name : swId;
            // Ruta de la imagen: images/software/ai.png, etc.
...
function renderCard(card) {
    ...
    const baseStyle = `background-color: ${card.baseBg};`;
    const cardStyle = `
    ...
    return `
        <div class="service-card-wrapper" data-card-id="${escapeHTML(card.id)}">
            <div class="service-card-base" style="${baseStyle}"></div>
...
async function initServices() {
    servicesLog('Inicializando Servicios...');
    ...
    if (!data) {
        console.warn('[SERVICES] No se pudieron cargar los datos');
        return;
    }
    ...
    servicesLog('Servicios inicializado correctamente');
}
